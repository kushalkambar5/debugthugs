...
void main() {
  ...
  float normX = (gl_FragCoord.x - 0.5 * uResolution.x) / uResolution.x;
  float normY = (gl_FragCoord.y - 0.5 * uResolution.y) / (0.5 * uResolution.y);
  float fadeY = smoothstep(0.95, 0.2, abs(normY));

  float e = 0.06 + uIntensity * 0.94;
  float env = pow(max(cos(normX * PI), 0.0), uTaper);
  ...
  float alpha = clamp(lum, 0.0, 1.0) * uOpacity * fadeY;
